...
type GrowthAction = { id: string; insightId: string; title: string; description: string; outcomeScenario: string; ownerEmail: string | null; dueDate: string | null; priority: number | null; status: string };
        ...
        {!selectedCap || !history ? (
          ...
          <div className="max-w-3xl mx-auto px-6 py-6">
            ...
            {selectedCapId && <section className="workspace-card p-5 mb-4" aria-labelledby="decision-title">
              ...
              <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => createDecision("APPROVED")} disabled={decisionBusy} className="px-3 py-1.5 rounded-lg flowstate-success-button text-white text-xs disabled:opacity-50">Approve assessment</button><button type="button" onClick={() => createDecision("EVIDENCE_REQUESTED")} disabled={decisionBusy} className="px-3 py-1.5 rounded-lg border border-[var(--card-border)] text-xs disabled:opacity-50">Request evidence</button>{decisions[0] && <><button type="button" onClick={() => updateDecision("REOPEN")} disabled={decisionBusy} className="px-3 py-1.5 rounded-lg border border-[var(--card-border)] text-xs disabled:opacity-50">Reopen</button><button type="button" onClick={() => updateDecision("SIGN_OFF")} disabled={decisionBusy} className="px-3 py-1.5 rounded-lg flowstate-accent-button text-white text-xs disabled:opacity-50">Sign off</button></>}</div>
            </section>}

...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const perspectives = await prisma.maturityPerspective.findMany({ where: { id: { in: decision.sourcePerspectiveIds.length ? decision.sourcePerspectiveIds : undefined }, capabilityId: id, status: "APPROVED" }, orderBy: { createdAt: "asc" } });

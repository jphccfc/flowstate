...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const attachments = await prisma.tagAttachment.findMany({
    where: { organizationId, capturedInputId, status: { in: ["APPROVED", "SUGGESTED"] } },
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!attachmentId || (body?.action !== "approve" && body?.action !== "reject")) return NextResponse.json({ error: "attachmentId and approve or reject action are required" }, { status: 400 });
  const attachment = await prisma.tagAttachment.findFirst({ where: { id: attachmentId, organizationId } });
  if (!attachment) return NextResponse.json({ error: "Hashtag suggestion not found." }, { status: 404 });

  const status = body.action === "approve" ? "APPROVED" : "REJECTED";
  if (attachment.status !== "SUGGESTED") return NextResponse.json(attachment);
  const reviewed = await prisma.tagAttachment.update({
    where: { id: attachment.id },
    data: { status, reviewedBy: user.email ?? user.id, reviewedAt: new Date() },
  });

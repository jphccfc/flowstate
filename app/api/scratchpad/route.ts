...
export async function POST(req: NextRequest) {
  ...
  const note = await prisma.capturedInput.create({ data: { organizationId: org, type: "TEXT_NOTE", rawText: sanitizeRichText(typeof body.text === "string" ? body.text : ""), status: "TRANSCRIBED", meetingContextId: contextId } });
...
export async function PATCH(req: NextRequest) {
  ...
  const updated = await prisma.capturedInput.updateMany({ where: { id, organizationId: existing.organizationId, revision }, data: { rawText: sanitizeRichText(typeof body.text === "string" ? body.text : ""), meetingContextId: contextId, revision: { increment: 1 } } });

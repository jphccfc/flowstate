...
describe("AI rating validation proposals", () => {
  ...
  it("creates a provisional validation linked to the approved decision, evidence, and perspectives", async () => {
    ...
    const perspective = await prisma.maturityPerspective.create({ data: { capabilityId: capability.id, stakeholderType: "employee", assessorEmail: "employee@test.com", score: 1, originalStatement: "We do this manually", sourceEvidenceIds: [tag.id], status: "APPROVED", reviewedBy: "reviewer@test.com", reviewedAt: new Date() } });
    const submittedPerspective = await prisma.maturityPerspective.create({ data: { capabilityId: capability.id, stakeholderType: "manager", assessorEmail: "manager@test.com", score: 2, originalStatement: "We have a draft process", sourceEvidenceIds: [tag.id], status: "SUBMITTED" } });
    const rejectedPerspective = await prisma.maturityPerspective.create({ data: { capabilityId: capability.id, stakeholderType: "executive", assessorEmail: "executive@test.com", score: 3, originalStatement: "We have a mature process", sourceEvidenceIds: [tag.id], status: "REJECTED", reviewedBy: "reviewer@test.com", reviewedAt: new Date(), reviewNotes: "Insufficient evidence" } });
    const decision = await prisma.assessmentDecision.create({ data: { capabilityId: capability.id, status: "APPROVED", score: 2, rationale: "Existing approved score", sourceEvidenceIds: [tag.id], sourcePerspectiveIds: [perspective.id, submittedPerspective.id, rejectedPerspective.id] } });

...
describe("assessment task organization authorization", () => {
  ...
  it("rejects linked context from another organisation", async () => { const otherOrg = await prisma.organization.create({ data: { name: "Other Task Organisation" } }); const domain = await prisma.businessDomain.create({ data: { organizationId: otherOrg.id, name: "Other domain" } }); const capability = await prisma.capability.create({ data: { domainId: domain.id, name: "Other capability" } }); const response = await POST(request({ type: "INTERVIEW", title: "Interview", description: "Interview owner", dueDate: "2030-01-01", linkedCapabilityId: capability.id }) as unknown as NextRequest, { params: Promise.resolve({ id: organizationId }) }); expect(response.status).toBe(400); await prisma.organization.delete({ where: { id: otherOrg.id } }); });

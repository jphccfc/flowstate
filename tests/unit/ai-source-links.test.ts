...
describe("AI Hub source links", () => {
  it("maps known source kinds to the current workspace internal routes", () => {
    ...
    expect(sourceHref("workspace-1", "document", "document-1")).toBe("/clients/workspace-1/documents/document-1");
  ...
  it("never treats source data as an external or cross-workspace URL", () => {
    expect(sourceHref("workspace/evil?next=https://example.com", "document", "id?x=https://evil.example")).toBe("/clients/workspace%2Fevil%3Fnext%3Dhttps%3A%2F%2Fexample.com/documents/id%3Fx%3Dhttps%3A%2F%2Fevil.example");
  });
});
